...
internal static class AhpNative
{
    ...
    /// <summary>Reads a required UTF-8 C string; throws if null.</summary>
    private static string In(IntPtr p, string name)
    {
    ...
    {
        try { ClearError(); return Store(Ahp.AhpState.Default()); }
    ...
    /// <summary>Applies one action in place. Returns 0 on success, -1 on error.</summary>
    ...
    public static int StateApply(long handle, IntPtr actionJson)
    {
        try
        {
            ...
            var action = Ahp.__default.decodeAhpAction(
                ParseStrict(In(actionJson, "action_json"), "action_json"));
            var next = Ahp.__default.applyAhp(state, action);
            lock (_statesLock) { _states[handle] = next; }
            return 0;
        }
    ...
    {
        try { ClearError(); return Load(a).Equals(Load(b)) ? 1 : 0; }
        catch (Exception ex) { SetError(ex.Message); return -1; }
    }
    ...
    {
        lock (_statesLock) { _states.Remove(handle); }
    }
}
