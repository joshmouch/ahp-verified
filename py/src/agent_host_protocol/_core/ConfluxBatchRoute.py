...
class default__:
    ...
    @staticmethod
    def BatchRoute(keyOf, op, m, batch):
        return ConfluxContract.default__.Fold(ConfluxPrune.default__.PrunableRoute(keyOf, op), m, batch)

    @staticmethod
    def Reset(seed):
        def lambda0_(d_0_seed_):
            def lambda1_(d_1_m_, d_2_a_):
                return d_0_seed_

            return lambda1_

        return lambda0_(seed)
