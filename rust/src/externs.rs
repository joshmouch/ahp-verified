...
//!
//! **This module is the host-effect trust boundary.**
//!
//! Process execution, filesystem, clocks, sockets, HTTP.
//! Dafny declares them `replaceable` / `{:extern}` and
...
//!
//! This crate exposes the PURE, VERIFIED REDUCERS only. Reducers are total
//! functions from (state, action) to state; they perform no I/O and never reach
//! any function in this file. The stubs therefore panic loudly rather than
//! return fabricated data: a silent default here would be indistinguishable
//! from a real host effect and would corrupt any caller that did reach it.
//!
//! If you see a panic naming a capability below, it is a bug in this crate --
//! no public API path should be able to get here.
//!
...
#![allow(missing_docs)]
// The generated core resolves capability members through `_default::<Method>`,
// so these type names are fixed by the code generator, not chosen here.
#![allow(non_camel_case_types)]
// The wide return tuples below mirror the Dafny `{:extern}` signatures exactly.
// Factoring them into type aliases would obscure the correspondence that makes
// these stubs auditable against the runtime's declarations.
...
fn unavailable(what: &str) -> ! {
    panic!(
        "ahp-verified: host capability `{what}` is not implemented in this build. \
...
pub mod ConfluxCommandIdentityCapability {
    ...
    impl _default {
        pub fn CanonicalJsonBytes(value: &Rc<crate::generated::ConfluxCodec::Json>) -> Bytes {
            unavailable("ConfluxCommandIdentityCapability.CanonicalJsonBytes")
        }

        pub fn DecodeCanonicalJson(
            bytes: &Bytes,
        ) -> Rc<crate::generated::ConfluxCodec::Option<Rc<crate::generated::ConfluxCodec::Json>>> {
